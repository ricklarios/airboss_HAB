...
export const ValidateScreen =  ({ history, match }) => {
    ...
    useEffect(()  =>  {
        ...
        async function sendValidation(){
            ...
            const response = await fetch(`http://localhost:3001/users/validate/${code}`);
            const data = await response.json();
            console.log(data);
            if (data.status==='ok'){
                setLogin(true);
                setNameUser(`${data.name} ${data.lastname}`)
                setValues({...values, ok: 'Usuario activado correctamente', showOk:true})
                setTimeout(() => {
                    history.push(`/`);
                }, 3000);
            }else{
                setValues({...values, error:'No se ha podido activar la cuenta', showError: true})
            }
