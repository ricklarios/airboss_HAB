...
function ResetPasswordForm({code}) { 
  ...
  const handleChange = (prop) => (event) => {
    ...
    if(event.target.id === 'standard-basic-repeatPassword' && event.charCode === 13){
  ...
  async function onSubmitChangePwd(event) {
    ...
    if(!validator.isEmpty(values.password) && !validator.isEmpty(values.repeatPassword) && values.password===values.repeatPassword){
      const body = {
        recoverCode: code,
        newPassword: values.password,
      }
      try {
        const res = await axios.put('http://localhost:3001/users/password/reset', body);
        
        if (res.data.status === 'ok'){
          setValues({...values, ok: "Nueva contraseña restaurada!", showOk: true});
          setTimeout(() => {
            history.push('/')
          }, 2000);
        }
      ...
      if(values.password!==values.repeatPassword){
        ...
        setValues({...values, error: 'Las contraseñas deben coincidir', showError: true});
      }
    }
  }
  ...
  return (
    <div className={"reset-password-form animate__animated " + animation} ref={ref}>
      ...
      <FormControl className={clsx(classes.margin, classes.textField)}>
                ...
                <FormControl className={clsx(classes.margin, classes.textField)}>
                    <InputLabel htmlFor="standard-adornment-password" className="label">repite contraseña</InputLabel>
                    <Input
        ...
        <div>
          <Button variant="contained" id="button-login" color="primary" className="button-form" onClick={onSubmitChangePwd}>
          Solicitar reseteo
          </Button>    
        </div>
        <>
